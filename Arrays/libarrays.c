...
if(array==NULL) return 0;
return (arraysGetLength(array)+1)*sizeof(AssocArray);
}
...
if(*index>=0){
  ...
  int nb=arraysGetLength(*array);
  *array=(void *)_realloc(*array,(nb+2)*sizeof(AssocArray));
  if(*array==NULL) return -1;
  bzero((*array)+nb,2*sizeof(AssocArray));
...
while((array[i].flags&AARRAY_END_OF_ARRAY)==0){
  ...
  if((array[i].flags&AARRAY_NUMERIC_INDEX)!=0){
    sprintf(number,"%d",(int)array[i].index);
    index=number;
    flags |= AARRAY_FORCE_NUMERIC;
    }
  else index=array[i].index;
  if((array[i].flags&AARRAY_DONT_FREE)!=0 &&
     (array[i].flags&AARRAY_COMPACT_VALUE)==0){
    if(full && array[i].size>0){
      data=malloc(array[i].size);
      memcpy(data,array[i].data,array[i].size);
      }
    else data=array[i].data;
    flags |= AARRAY_DONT_DUPLICATE;
    }
  else{
    if((array[i].flags&AARRAY_COMPACT_VALUE)!=0)
      data=(void *)&(array[i].data);
    else{
      data=array[i].data;
      if(array[i].size<=sizeof(void *)) flags |= AARRAY_DONT_COMPACT;
      }
    }
  arraysSetValue(&result,index,data,array[i].size,flags);
  i++;
  }
return result;
}
...
int size=arraysGetLength(array);
